    ...
    while video_capture.isOpened():
        ...
        for face_encoding, face_location in zip(face_encodings, face_locations):
            ...
            if True in matches:
                ...
                name = input(f"Yangi odamni ismni kiriting (Person {new_person_index}): ")
                insert_person(name, face_encoding)  # Yuz encodingini ma'lumotlar bazasiga saqlash
                known_encodings.append(face_encoding)  # Yuz encodingini saqlash
                known_names.append(name)  # Odamni nomi
                new_person_index += 1
